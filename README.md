 ...
 This configuration targets the Houdini HOU-GEMM backend on HiveOS. It is intended for rigs running a single NVIDIA GPU with the canonical proof geometry.
 ...
 Notes
   ...
   • This configuration assumes m = 196608, n = 196608, k = 2048, rank = 128 as the fallback proof geometry.
   • It has 2% dev fee, disables local ZK, and runs a single consumer slot with blocking CUDA sync.
   • Adjust  PEARL_HOU_GEMM_BATCH_VRAM_TARGET_MB  if your GPU has less than 8 GB of VRAM.
   • For multi-GPU rigs, set  PEARL_SINGLE_WORKER  and consumer slot counts accordingly.
